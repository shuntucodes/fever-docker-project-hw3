...
class SampledSandeepReader(Reader):
    def __init__(self, fever_home=FEVER_HOME, **kwargs):
        # src_filename = os.path.join(
        #     fever_home, "train_sampled.jsonl")
...
def build_dataset(reader, phi, oracle, vectorizer=None, vectorize=True):
    ...
    labels = []
    raw_examples = []
    raw_examples_dict = defaultdict(dict)
    index=0
    ...
    for ex in tqdm(reader.read(), total=total_len, unit="examples", desc='Reading from dataset'):
        ...
        claim = ex.claim
        raw_examples_dict['claim']=ex.claim
        ev_ids = ex.get_evidence_ids()
        sents = [oracle.get_sentence(ev_id[0], ev_id[1]) for ev_id in ev_ids]
        raw_examples_dict['sents'] = sents
        d = phi(claim, sents)
        feats.append(d)
        labels.append(ex.label)
        raw_examples_dict['label'] = ex.label
        index+=1
        raw_examples.append((claim, sents, ex.label))
...
def build_dataset_sandeep(reader, phi, vectorizer=None, vectorize=True):
    """Create a dataset for training classifiers using `sklearn`.
...
def experiment(
    ...
    assess = build_dataset_sandeep(
        ...
        vectorize=vectorize)
    X_assess, y_assess = assess['X'], assess['y']
    # with(open('dev_ev_text.json','w+')) as fp:
    #     json.dump(assess['raw_examples'],fp)
    # print("got assesment")
    # Train:
